...
#include <cmath>       // Header file for trigonometric functions.
...
public:
	vertex(){};
  vertex(float set_x, float set_y, float set_z, float set_u, float set_v)
	{
		set(set_x, set_y, set_z, set_u, set_v);
	}
	~vertex () {};
  set(float set_x, float set_y, float set_z, float set_u, float set_v)
	{
		x = set_x;
		y = set_y;
		z = set_z;
...
class triangle 
{
	vertex vertices[3];
public:
	triangle (vertex* set_vertices)
	{
		for (int i = 0; i < 3; i++)
		{
			vertices[i].set(set_vertices[i]);
		}
	};
	~triangle (){};
};
...
void world::DrawGLScene()
{
    ...
    for (loop=0; loop<triangle_count; loop++) {        // loop through all the triangles
	glBegin(GL_TRIANGLES);		
	glNormal3f( 0.0f, 0.0f, 1.0f);
	
	x_m = sector1.triangles[loop].vertices[0].x;
	y_m = sector1.triangles[loop].vertices[0].y;
	z_m = sector1.triangles[loop].vertices[0].z;
	u_m = sector1.triangles[loop].vertices[0].u;
	v_m = sector1.triangles[loop].vertices[0].v;
	glTexCoord2f(u_m,v_m); 
	glVertex3f(x_m,y_m,z_m);
	
	x_m = sector1.triangles[loop].vertices[1].x;
	y_m = sector1.triangles[loop].vertices[1].y;
	z_m = sector1.triangles[loop].vertices[1].z;
	u_m = sector1.triangles[loop].vertices[1].u;
	v_m = sector1.triangles[loop].vertices[1].v;
	glTexCoord2f(u_m,v_m); 
	glVertex3f(x_m,y_m,z_m);
	
	x_m = sector1.triangles[loop].vertices[2].x;
	y_m = sector1.triangles[loop].vertices[2].y;
	z_m = sector1.triangles[loop].vertices[2].z;
	u_m = sector1.triangles[loop].vertices[2].u;
	v_m = sector1.triangles[loop].vertices[2].v;
	glTexCoord2f(u_m,v_m); 
	glVertex3f(x_m,y_m,z_m);	
